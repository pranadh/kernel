...
Cree un archivo `***REMOVED***` en la raíz de su proyecto:
...
console.log(process***REMOVED***) // elimine esto después que haya confirmado que esta funcionando
```
...
Eso es todo. `process***REMOVED***` ahora tiene las claves y los valores que definiste en tu archivo `***REMOVED***`:
...
```bash
$ node -r dotenv/config tu_script.js dotenv_config_path=/custom/path/to/***REMOVED*** dotenv_config_debug=true
```
...
```bash
$ DOTENV_CONFIG_ENCODING=latin1 DOTENV_CONFIG_DEBUG=true node -r dotenv/config tu_script.js dotenv_config_path=/custom/path/to/***REMOVED***
```
...
Necesitas mentener sincronizados los archivos `***REMOVED***` entre maquinas, entornos, o miembros del equipo? Usa 
...
`Configuración` leerá su archivo `***REMOVED***`, analizará el contenido, lo asignará a [`process***REMOVED***`](https://nodejs.org/docs/latest/api/process.html#process_process_env),
...
Por defecto: `path.resolve(process.cwd(), '***REMOVED***')`
...
```js
require('dotenv').config({ path: '/personalizado/ruta/a/***REMOVED***' })
```
...
Anule cualquier variable de entorno que ya se haya configurada en su maquina con los valores de su archivo ***REMOVED***.
...
### ¿Por qué el archivo `***REMOVED***` no carga mis variables de entorno correctamente?

Lo más probable es que su archivo `***REMOVED***` no esté en el lugar correcto. [Vea este stack overflow](https://stackoverflow.com/questions/42335016/dotenv-file-is-not-loading-environment-variables).
...
### ¿Debo confirmar mi archivo `***REMOVED***`?

No. Recomendamos **enfáticamente** no enviar su archivo `***REMOVED***` a la versión de control. Solo debe incluir los valores especificos del entorno, como la base de datos, contraseñas o claves API.

### ¿Debería tener multiples archivos `***REMOVED***`?

No. Recomendamos **enfáticamente** no tener un archivo `***REMOVED***` "principal" y un archivo `***REMOVED***` de "entorno" como `***REMOVED***.test`. Su configuración debe variar entre implementaciones y no debe compartir valores entre entornos.
...
Por defecto, nunca modificaremos ninguna variable de entorno que ya haya sido establecida. En particular, si hay una variable en su archivo `***REMOVED***` que colisiona con una que ya existe en su entorno, entonces esa variable se omitirá.

Si por el contrario, quieres anular `process***REMOVED***` utiliza la opción `override`.
...
Su código React se ejecuta en Webpack, donde el módulo `fs` o incluso el propio `process` global no son accesibles fuera-de-la-caja. El módulo `process***REMOVED***` sólo puede ser inyectado a través de la configuración de Webpack.
...
Sí! `dotenv.config()` devuelve un objeto que representa el archivo `***REMOVED***` analizado. Esto te da todo lo que necesitas para poder establecer valores en `process***REMOVED***`. Por ejemplo:
...
export default new Client(process***REMOVED***.CLAVE_API)
...
`process***REMOVED***.CLAVE_API` será vacio.
...
### ¿Qué pasa con la sincronización y la seguridad de los archivos ***REMOVED***?
