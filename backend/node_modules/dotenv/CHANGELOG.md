...
- 🐞 Fix recent regression when using `path` option. return to historical behavior: do not attempt to auto find `***REMOVED***` if `path` set. (regression was introduced in `16.4.3`) [#814](https://github.com/motdotla/dotenv/pull/814)
...
- Add `error.code` to error messages around `***REMOVED***.vault` decryption handling [#795](https://github.com/motdotla/dotenv/pull/795)
- Add ability to find `***REMOVED***.vault` file when filename(s) passed as an array [#784](https://github.com/motdotla/dotenv/pull/784)
...
- Optionally pass `DOTENV_KEY` to options rather than relying on `process***REMOVED***.DOTENV_KEY`. Defaults to `process***REMOVED***.DOTENV_KEY` [#754](https://github.com/motdotla/dotenv/pull/754)

## [16.2.0](https://github.com/motdotla/dotenv/compare/v16.1.4...v16.2.0) (2023-06-15)

### Added

- Optionally write to your own target object rather than `process***REMOVED***`. Defaults to `process***REMOVED***`. [#753](https://github.com/motdotla/dotenv/pull/753)
...
- Add `***REMOVED***.vault` support. 🎉 ([#730](https://github.com/motdotla/dotenv/pull/730))

ℹ️ `***REMOVED***.vault` extends the `***REMOVED***` file format standard with a localized encrypted vault file. Package it securely with your production code deploys. It's cloud agnostic so that you can deploy your secrets anywhere – without [risky third-party integrations](https://techcrunch.com/2023/01/05/circleci-breach/). [read more](https://github.com/motdotla/dotenv#-deploying)
...
- _Breaking:_ `#` marks the beginning of a comment (UNLESS the value is wrapped in quotes. Please update your `***REMOVED***` files to wrap in quotes any values containing `#`. For example: `SECRET_HASH="something-with-a-#-hash"`).

..Understandably, (as some teams have noted) this is tedious to do across the entire team. To make it less tedious, we recommend using [dotenv cli](https://github.com/dotenv-org/cli) going forward. It's an optional plugin that will keep your `***REMOVED***` files in sync between machines, environments, or team members.
...
- Log error on failure to load `***REMOVED***` file ([#594](https://github.com/motdotla/dotenv/pull/594))
...
- _Breaking_: default `path` is now `path.resolve(process.cwd(), '***REMOVED***')`
- _Breaking_: does not write over keys already in `process***REMOVED***` if the key has a falsy value
...
- Silent option to silence `console.log` when `***REMOVED***` missing

## [1.0.0] - 2015-03-13

### Removed

- support for multiple `***REMOVED***` files. should always use one `***REMOVED***` file for the current environment
